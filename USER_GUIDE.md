...
### 8.5 GET `/health`
- Возвращает `{"service": "TORO Maintenance Orders API", "status": "ok", "spec_version": "Opus 4.5", "uptime_seconds": 42}`.
- Поля `service` и `uptime_seconds` позволяют быстро определить, какой инстанс отвечает и сколько он работает.
