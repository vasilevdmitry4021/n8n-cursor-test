...
### 8.5 GET `/health`
- Возвращает `{"status": "ok", "spec_version": "Opus 4.5", "uptime_seconds": 42}`.
- Поле `uptime_seconds` показывает длительность работы процесса и полезно для мониторинга рестартов.
